...
for(i in 1:100){
  ...
  my_fpath <- file.path(".",args[3], paste0("ci_train_",toString(i)))
  dir.create(my_fpath)
  
  write.csv(training_set, file.path(my_fpath, args[4]), row.names=FALSE)
  ...
  write.csv(final_data, file.path(my_fpath,"true_accuracy.csv"))
